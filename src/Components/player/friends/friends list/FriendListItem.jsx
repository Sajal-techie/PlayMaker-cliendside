...
const FriendListItem = ({ name, bio, id, profileImage, type, fetchData, suggestion }) => {
  ...
  return (
    <div className="flex items-center p-2 sm:p-4 bg-white shadow-md rounded-lg mb-4 font-kanit relative">
      <img
      ...
      <div className="ml-4 cursor-pointer" onClick={() => handleNavigate(id)}>
        <h3 className="text-lg font-semibold overflow-clip overflow-ellipsis max-w-20 sm:max-w-none hover:underline">{name}</h3>
      ...
      { 
        ...
        <>
          <div className="ml-auto flex space-x-2 relative">
            <button className="sm:px-4 px-2 py-1 bg-blue-500 text-white rounded flex items-center" onClick={()=>navigate(`/profile/${id}`)}>
              <span className=''>view <br className='sm:hidden' /> profile</span> &nbsp;
            </button>
          </div>
        </>
      :

        <div className="ml-auto flex space-x-1 relative">
        <button className="md:px-4 px-1 py-2 bg-blue-500 text-white rounded flex items-center" onClick={()=>handleMessageClick()}>
        ...
        <button className="md:px-4 px-2 py-2 bg-gray-300 rounded relative" onClick={handleDropdownToggle}>...</button>
        {dropdownVisible && (
