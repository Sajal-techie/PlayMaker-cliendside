...
const MainSection = ({id,username,bio,state,district,academy,profile_pic,fetchapi}) => {
    ...
    const img = academy ? "https://blog.hubspot.com/hs-fs/hubfs/Hubspot-fb%20cover.jpg?width=650&height=275&name=Hubspot-fb%20cover.jpg" : "https://vojislavd.com/ta-template-demo/assets/img/profile-background.jpg"
    const profileImgSrc = profile_pic ? baseUrl+profile_pic: "https://t4.ftcdn.net/jpg/00/64/67/27/360_F_64672736_U5kpdGs9keUll8CRQ3p3YaEv2M6qkVY5.jpg"
    
    //  tp  view profile photo
    const ViewProfilePhoto = (image,name,cover)=>{
        document.getElementById('my_modal_3').showModal(), 
        
        profile_pic ? setProfile([`${image}`,name]) :setProfile([`${image}`,name])
    }
    return (
    <>
        <div className="bg-white rounded-lg shadow-xl pb-2">
        {/* cover pic edit  */}
            <div className="absolute  right-0  sm:right-14 md:right-20 xl:right-44  mt-4 rounded mr-2" onClick={()=>ViewProfilePhoto(img)}>
                <button  className="border border-black hover:border-white p-2 rounded text-white hover:text-gray-800 bg-gray-100 bg-opacity-10 hover:bg-opacity-20" title="change cover picture">
            ...
            {/* cover pic */}
            <div className="w-full h-[250px] bg-slate-300"> 
                <img src={img} className="w-full h-full rounded-tl-lg rounded-tr-lg"/>
            </div>
            {/* cover pic end  */}

            <div className={`${imgDivClass} flex flex-col -mt-20 font-kanit capitalize`}>                 
                    <img src={profileImgSrc} style={{borderRadius:'50%'}} className="w-44 h-44 object-cover  border-4 border-white " onClick={()=>ViewProfilePhoto(profileImgSrc)} />                    
